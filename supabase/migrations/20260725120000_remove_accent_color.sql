-- ============================================================================
-- Vesper — Migration: rimozione completa di accent_color
-- ----------------------------------------------------------------------------
-- Con gli avatar DiceBear l'SVG copre l'intera bolla, quindi il colore di
-- sfondo personalizzato (accent_color) non è più né scegliibile né visibile.
-- Rimuoviamo la colonna e la sua esposizione da view/funzioni.
--
-- La colonna è referenziata da:
--   * view public.public_profiles
--   * function public.search_users(...)
-- Vanno ricreate prima di poter droppare la colonna.
-- ============================================================================

-- 1) public_profiles — stessa definizione corrente, senza accent_color.
--    (non si può togliere una colonna con CREATE OR REPLACE VIEW: drop+create)
...
-- 2) search_users — stessa firma/logica, senza accent_color nel RETURNS TABLE
--    e nella select. Il tipo di ritorno cambia: drop + create.
...
-- drop+create azzera i grant: riassegno l'execute a authenticated (firma
-- dei parametri invariata, cambia solo il RETURNS TABLE).
...
-- 3) Ora la colonna non è più referenziata: la si può rimuovere.
